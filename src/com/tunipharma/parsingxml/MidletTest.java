...
public class MidletTest extends MIDlet implements CommandListener, Runnable {
    ...
    Form form = new Form("INFO PHARMACIE");
    ...
    public void run() {
        ...
        if (x) {
            try {
                ...
                HttpConnection hc = (HttpConnection) Connector.open("http://localhost/PIDEV/getXmlfGouv.php");
...
//                        if (no.getString().toUpperCase().equals(pharmacies[i].getNom().toUpperCase())) {
                ...
                try {
                    ...
                    // get an InputStream from somewhere (could be HttpConnection, for example)
                    HttpConnection hc2 = (HttpConnection) Connector.open("http://localhost/PIDEV/getXmlService.php");
                ...
                try {   // this will handle our XML
                    ...
                    // get an InputStream from somewhere (could be HttpConnection, for example)
                    HttpConnection hc1 = (HttpConnection) Connector.open("http://localhost/PIDEV/getXmlGarde.php");
